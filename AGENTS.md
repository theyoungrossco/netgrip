...
## Releasing (bump the version at every milestone)

The version lives in **two** files that must always agree:
`src/netgrip/__init__.py` (`__version__`) and `pyproject.toml` (`version`).
`scripts/release.sh` refuses to run if they drift.

**When a milestone (0.x) lands, cut a release — don't let work pile up under
`[Unreleased]` across several milestones.** (We learned this the hard way: 0.2's
persistence work was never tagged, so it had to ship folded into 0.3.0.) Each
release, in order:

1. Bump the version in **both** files above.
2. Move the `[Unreleased]` items in `CHANGELOG.md` under a new `## [X.Y.Z] -
   <date>` heading and fix the compare links at the bottom.
3. Add a `<release version="X.Y.Z" date="…">` entry to
   `data/io.github.theyoungrossco.netgrip.metainfo.xml` (AppStream validators
   check it).
4. Run `scripts/release.sh --tag --push`. It lints, tests, builds the Linux
   sdist/wheel, creates the `vX.Y.Z` tag and pushes it — which triggers
   `.github/workflows/release.yml` to build the **Linux artifacts and the Windows
   `setup.exe`** and publish a GitHub Release.

Installers (Linux desktop integration + Windows `setup.exe`) are regenerated by
that tag-triggered workflow, so they never need hand-building per release. See
